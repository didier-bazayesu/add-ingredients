...
function Home() {
  ...
  //showing the user the remaining ingredients fo making some meanl
  const remaining = 4 - ingredient.length;
  const readyMeal =
  remaining > 0
  ? `${remaining} ingredient${remaining > 1 ? "s" : ""} remaining to prepare the meal.`
      ...
      <section className="bg-white/30 backdrop-blur-md p-10 rounded-3xl shadow-2xl w-full max-w-4xl mx-auto border border-white/20">
        ...
        {ingredient.length > 0 && (
          <div>
            ...
            {/* Recipe Section */}
            <h1 className='pb-5 text-amber-500'>{readyMeal}</h1>
            {ingredient.length > 3 && (
